"""
Lesnov Voice — Simple Recorder
- Экран + микрофон (один поток)
- Громкий чистый звук
"""
import os
import subprocess
...
class ScreenRegionSelector:
    ...
    def showFullScreen(self):
        w, h = get_screen_size()
        if self.callback:
            self.callback({"left": 0, "top": 0, "width": w, "height": h})
...
class MeetingRecorder:
    ...
    def start(self, region=None, mic_device=None, record_system=False):
        ...
        w, h = get_screen_size()
        print(f"[Rec] Started: {self._base_name}")
        return True
    ...
    def _save(self):
        ...
        for f in self._frames:
            out.write(f)
        out.release()
        print(f"[Rec] Video: {len(self._frames)} frames")
        
        # Save audio with volume boost
        if self._audio:
            ...
            print(f"[Rec] Audio: {len(arr)/self.rate:.1f}s")
        
        # FFmpeg merge with loud audio
        ffmpeg = get_ffmpeg()
